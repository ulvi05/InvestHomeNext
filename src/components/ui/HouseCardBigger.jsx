...
const HousesList = ({ house, customWidth }) => {
  ...
  const sliderSettings = {
    ...
    slidesToShow: 1,
    slidesToScroll: 1,
    speed: 200, // Added speed for faster slide change
    autoplay: false,
    autoplaySpeed: 3000,
    draggable: false,
    swipe: false,
...
<div className="relative">
  <Slider {...sliderSettings} className="rounded-t-[0.5rem]">
    {house.images.map((imgSrc, index) => (
      <div className="outline-none relative aspect-[4/3]" key={index}>
        <Image
          src={imgSrc}
          alt={`${house.title} image`}
          fill
          sizes="(max-width: 950px) 100vw, (max-width: 1200px) 50vw, 25vw"
  ...
  {/* Positioning icons absolutely over the image */}
  <div className="absolute bottom-[12px] left-[8px] z-10 px-[10px] py-[3px] rounded-[0.5rem] bg-[#fff] flex items-center gap-[3px] justify-center w-max">
    <Image src={MetroIcon} alt="Metro Icon" width={16} height={6} className="w-[18px]" />
    <span className="text-[10px] font-[400] text-[#808080]">N.Nərimanov</span>
  </div>

  <div className="absolute bottom-[12px] right-[8px] z-10 flex items-center gap-[4px] rounded-[0.5rem] px-2 py-1">
    <Image src={VideoSvg} alt="Video Icon" width={14} height={14} />
    <span className="text-white text-[10px] font-medium leading-[15px]">1</span>
  </div>
</div>
 ...
 <div className="absolute z-[1000] top-[13px] left-[11px] flex flex-col gap-[6px]">
